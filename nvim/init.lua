  ...
  { -- LSP Configuration & Plugins
    ...
    config = function()
      ...
      local servers = {
        ...
        gopls = {},
        pyright = {},
        ts_ls = {},  -- TypeScript/JavaScript LSP (lspconfig name)
        -- rust_analyzer = {},
      ...
      -- Filter out ts_ls and pyright since they're installed globally, not via Mason
      local filtered_installed = {}
      for _, pkg in ipairs(ensure_installed) do
        if pkg ~= 'ts_ls' and pkg ~= 'pyright' then
          table.insert(filtered_installed, pkg)
        end
      end
      ...
      -- Setup ts_ls manually since we're not using Mason for it
  ...
  { -- Highlight, edit, and navigate code
    ...
    config = function()
      ...
      require('nvim-treesitter.configs').setup {
        ensure_installed = { 'bash', 'c', 'go', 'html', 'javascript', 'lua', 'markdown', 'python', 'typescript', 'vim', 'vimdoc' },
        -- Autoinstall languages that are not installed
        auto_install = true,
        highlight = { enable = false },  -- Disable for now due to compatibility issues
        indent = { enable = true },
      }
